...
But most implementation use a `TreeMap<Character, State>` to store the *goto* structure, which costs `O(ln(t))` time, `t` is the largest amount of a word's common suffixes. The final complexity is O(n * ln(t)), absolutely `t > 2`, so n * ln(t) is greater than n. The others used a `HashMap`, which wasted too much memory, and still remained slowly.
